...
and robust access.
On the other hand, they concentrate too much power over its users, holding and
monetizing over their private data and behavior.
%
...
illicit content (e.g., hate speech).
...
\footnote{
    Note that to some extent AI content may be indistinguishable from human
    content, but its frequency and relevance in forums is still subject to
    human evaluation in our proposal.
}
...
still all connected.
%
We assume that within the common prefix, users $a$ and $b$ have contributed
with better content and therefore have more reputation combined than $c$ has
alone (i.e., $8+5>3$).
We will use this information when ordering branches in the consensus algorithm.
%
Then, user $c$ disconnects from $a$ and $b$, and evolves in separate along with
new users $x$ and $y$ with posts in \code{branch-1}.
In the meantime, $a$ and $b$ participate with posts in \code{branch-2}.
...
merging branches.
For instance, dislike operations in one branch to common past posts could
possibly block posts from users in the other branch, depending on how they are
ordered.
...
Therefore, based on the reputation of users in the common prefix,
Figure~\ref{fig.reps}.B indicates consensus order between posts in the forum
DAG:
...
%
Peers that first received branches with less reputation need to reorder all
posts starting at the forking point.
...
%
Note that the consensus order exists for reputation accountability purposes,
...
When merging, these fake users become legitimate and now accumulate the
...
\code{branch-1} and post extra dislikes to user $c$ in \code{branch-2} so that
a further merge invalidates all posts from \code{branch-1}, removing them from
the DAG.
%
...
In any case, it is unacceptable for correct users that a very old remote branch
affects a long active forum.
...
A reasonable policy is to hold blocked posts in a temporary but public bag and
...
Because the protocol relies on a tamper-proof DAGs, and because each peer
self verifies the posts as they synchronize, the protocol becomes tolerant to
...
service attacks in the network, they cannot modify the DAG structure, create
arbitrary content (without performing work), or impede that two correct nodes
...
are discussed as follows through an example.
...
practicability of \FC and its consensus algorithm.
%
