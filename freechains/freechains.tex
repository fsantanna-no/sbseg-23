...
\subsection{Synchronization and Byzantine Peers}
...
the messages.
When peers synchronize, they mutually synchronize missing branches such that
they reach the same DAG structure~\cite{p2p.byz}.
...
Byzantine-faulty peers~\cite{lamport.byz}.
%
No information coming from any peer is trusted a priori.
%
...
Therefore, although Byzantine peers may still perform some kind of denial of
service attacks in the network, they cannot modify the DAG structure, create
new content (without performing work), or impede that two correct nodes
...
chains.
Fully peer-to-peer systems cannot rely on logins or {\scriptsize CAPTCHAs}
