...
content, including harmful texts and images~\cite{btc.content}.
%~\cite{btc.data}
...
(i.e., $8+5>3$).
We will use this fact when ordering branches in the consensus algorithm.
%
Then, user $c$ disconnects from $a$ and $b$, and evolves with new users $x$ and
$y$ with posts in \code{branch-1}.
...
Figure~\ref{fig.reps}.B indicates the consensus order between posts in the forum
DAG:
...
operations, and is only a view of the primary DAG structure.
...
the causal relationships between the messages.
%
